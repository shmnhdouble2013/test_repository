...
KISSY.add('act/double11-come-on/spikectrl', function(S){
        ...
        S.augment(SpikeCtrl, {
                ...
                // 获取小时 时间段
                _getHourLength: function(el){
                    var _self = this,
                        nextEl = S.one(el).next(),
                        nextHour = parseInt(DOM.attr(nextEl, BLOCK_DATA_TIME), 10),
                        curHour = parseInt( DOM.attr(el, BLOCK_DATA_TIME), 10);

                    if(_self.get('isCustomTimePeriod')){
                       return nextHour - curHour;
                    }else{
                       return _self.get('hourLength'); 
                   }
                },

                // 根据 容器 个数 和 data-hour 时间 配置参数, 显示 时间文本 和 data-hour 标记  --- 整点
                renderTimeBlock: function(){
                    var _self = this;

                    S.each(_self.timeBlock, function(el, num){
                        var num = num + 1,
                            hourTimeLength = _self._getHourLength(el), 
                            hoursContainer = S.one(el).first(_self.get('hoursContainerCls')),
                            timeRange = num*hourTimeLength - hourTimeLength,
                            timeText = _self.getFullTimeStr(timeRange) + ':00';
                        
                        if(timeRange > 23){
                            return;
                        }  

                        DOM.attr(el, BLOCK_DATA_TIME, timeRange);
                ...
                _renderStateAll: function(el, index){
                    var _self = this, 
                        hourLength = DOM.attr(el, BLOCK_DATA_TIME),
                        curDateStr = _self.dataYMD +' '+ _self.getFullTimeStr(hourLength) + ':00:00',

                        spikeTimeStart = _self.getDateParse(curDateStr),
                        spikeTimeEnd = _self.offsetDateSeconds(curDateStr, parseInt(hourLength, 10), '+'),                       

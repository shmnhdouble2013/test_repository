	...
	*/
	...
	S.extend(Base, O.Dialog, {
		...
		renderUI: function () {
			...
			this.get('body').addClass('ks-clear').append('<div class="' + prefixCls + CLS_PREFIX + 'icon"><i></i></div><div class="' + prefixCls + CLS_PREFIX + 'content"></div>');
		},
		_uiSetTitle: function (title) {

			if (title) {
				this.get('header').html(title);
			}
		},
		_uiSetIcon: function (icon) {

			var prefixCls = this.get('prefixCls'),
				messageIcon = this.get('body').one('.' + prefixCls + CLS_PREFIX + 'icon');
			messageIcon[0].className = prefixCls + CLS_PREFIX + 'icon';//清除class 还原为初始值
			messageIcon.addClass(prefixCls + CLS_PREFIX + icon);
		},
		...
		_uiSetButton: function (buttons) {
			...
			if (buttons && buttons.length > 0) {
				...
				S.each(buttons, function (item, index) {
					button = new Node('<button class="' + prefixCls + CLS_PREFIX + 'btn">' + item.text + '</button>').appendTo(footer);
					if (index === 0) {
...
KISSY.add(function (S, Base) {
			...
			*/
			...
			_buildParam: function (arg) {
				...
				case 2:
					...
					else if ('info,error,right,question,warning'.indexOf(arg[0]) !== -1) {
						param['icon'] = arg[0];
						param['message'] = arg[1];
					}
					else {
    ...
    requires: ['message/base', './css/message.css']
});

...
KISSY.add(function(S){
	...
	*/
	...
	var UI_SET = '_uiSet',
        ...
        CLS_ICON_BUTTON = 'bar-icon-button',
	...
	S.augment(buttonBarItem,{
		attachEvent : function(){
			var _self = this,
				el = _self.get('el');
			buttonBarItem.superclass.attachEvent.call(_self);
			if(el){
		...
		_getItemTemplate : function(){
			var _self = this,
                ...
                iconTpl = _self._getIconTpl(_self.icon),
                iconCss = iconTpl ? CLS_ICON_BUTTON : '',
				temp = ['<div data-id="',_self.id,'" class="', CLS_BTN_CONTAINER, ' ', clsDisable, ' ', _self.containerCss,' "><button class="bar-item-btn ',iconCss,'" autocomplete="off" hidefocus="true" ', disabledText, ' type="button">',iconTpl, _self.text, '</button></div>'].join('');
			return temp;
		},
        _getIconTpl :function(icon){
            if(icon){
               return ' <div class="x-icon x-icon-small x-icon-'+icon.type+'">'+icon.content+'</div>'
            }
            return '';
        },
    ...
    var pagingButtonBarItem = function(config){
		var _self = this;
		pagingButtonBarItem.superclass.constructor.call(_self, config);
		
	};
	S.extend(pagingButtonBarItem,buttonBarItem);
    S.augment(pagingButtonBarItem,{
		_getItemTemplate : function(){
			var _self = this,
				clsDisable =  _self.disabled ? CLS_DISABLE : '',
				disabledText = _self.disabled ? 'disabled="disabled"' : '',
				clsCustom = _self.text ? CLS_BTN_CUSTOM : '',
				temp = ['<div id="',_self.id,'" class="', CLS_BTN_CONTAINER, ' ', clsDisable, ' ', _self.containerCss,'"><button class="', CLS_ITEM_BTN, ' pb-page-btn ', _self.css, '" autocomplete="off" hidefocus="true" ', disabledText, ' type="button">', _self.text, '</button></div>'].join('');
			return temp;
		}
    });
	...
	barItem.types = {
        ...
        pageButton : pagingButtonBarItem,
	...
	S.augment(bar,{
		...
		_getBarTemplate : function(){
			return '<div class="lp-bar-container"></div>';
		},
		//初始化配置项
		_init : function(){
			var _self = this,
		...
		_initDOM : function(){
			var _self = this,
				...
				container = S.isObject(renderTo) ? renderTo : DOM.get('#'+renderTo),
				items = _self.get('items'),
				barEl = new Node(barTemplate).appendTo(container);/*,
				tableEl = new Node('<table style="border-spacing: 0;table-layout: fixed;"><tr></tr></table>').appendTo(barEl),
	...
	*/
	...
	/** @lends S.LP.PaggingBar.prototype */	
	{
		...
		_getPageInfoConfig : function(){
			var temp = ['<div><span class="', CLS_IEXT_CONTAINER, ' ', CLS_TEXT_PAGE, '">共 0 页</span><span class="', CLS_IEXT_CONTAINER, '">第</span><span class="', CLS_INPUT_CONTAINER, '"><input type="text" autocomplete="off" class="', CLS_ITEM_INPUT, '" size="20" name="inputItem"> 页</span></div>'].join('');
...
}, {requires : ['core','./uicommon','./css/bar.css']});
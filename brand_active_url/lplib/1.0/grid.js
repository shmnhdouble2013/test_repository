...
KISSY.add(function (S) {
			...
			if(row){
				S.each(columns, function (column) {
					var field = column.dataIndex,
						...
						eitable = editor ? (editor.editableFun && !editor.editableFun(record[field],record) ? '' : CLS_CELL_EDITABLE) : '',
						cls = PREFIX_CLS_CELL + id,
						cell = S.one('.' + cls, row),
						textEl = cell.one('.' + CLS_CELL_TEXT),
						text = null;
					if(eitable){
						cell.addClass(CLS_CELL_EDITABLE);
					}else{
						cell.removeClass(CLS_CELL_EDITABLE);
					}
					if(textEl){
						text =  column.renderer ? column.renderer(record[field], record) : record[field];
	...
	*/
	...
	S.augment(selectGridEditor,{
		...
		_getTemplate : function(){
            var temp = [ '<select class="lp-form-select-field lp-editor-field" tabindex="1" id="sel',S.guid(),'"  style="height : 20px;"></select>',
                '<s class="', CLS_EDITOR_ERROR_ICON, '"></s>'].join('');
	...
	S.augment(dateGridEditor,{
		...
		/*_initEvent : function(){
			...
			if(!_self.required){
				//清除日期
				datepicker.on('dateclear',function(){
					 _self._valueChange(null,null);
				});
			}
			datepicker.on(_self.CHANGE_EVENT,function(ev){
				_self._valueChange(null,ev.date);
			});
		},*/
    ...
    requires : ['core', './uicommon','./bar', './css/grid.css']
});
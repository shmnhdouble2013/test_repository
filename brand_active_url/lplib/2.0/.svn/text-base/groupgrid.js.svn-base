...
KISSY.add(function(S){
	...
	*/
	var DOM = S.DOM,
		...
		CLS_GRID_ROW_BLANK = 'grid-row-blank',
	...
	*/
	...
	/** @lends  S.LP.GroupGrid.prototype */		
	{
		...
		_beforeShow: function(){
			var _self = this;
			_self._createBlankRow();
		},
		// 创建空行
		_createBlankRow: function(){
			var _self = this,
				body = _self.get('tbody'),
				rowTemplate,
				rowEl;	
			rowTemplate = _self._getBlankRowTemplate();
			rowEl = new Node(rowTemplate).appendTo(body);
            return rowEl;
		},
		// 空行模板
		_getBlankRowTemplate: function(){
			var _self = this,
			...
			if (checkable) {
				cellTemp =  _self._getBlankRowCheckedCellTemplate();
				cellTempArray.push(cellTemp);
			}
			S.each(cells, function (column) {
				var temp = _self._getBlankRowCellTemplate(column);
				cellTempArray.push(temp);
			});

			rowTemplate = ['<tr class="', CLS_GRID_ROW_BLANK, '">', cellTempArray.join(''), '</tr>'].join('');
			return rowTemplate;		
		},
		// 空行多选框列模板
		_getBlankRowCheckedCellTemplate: function(){
			return ['<td width="30px" align="center" class="', CLS_GRID_CELL, '"></td>'].join('');
		},
		// 空行列模板
		_getBlankRowCellTemplate: function(column){
			var _self = this,
				width = column.width,
				template = ['<td  class="', CLS_GRID_CELL, '" width="', width, 'px"></td>'].join('');
			return template;		
		},
		...
		_getGroupRowTemplate: function(obj){
			...
			S.each(groupManager.columns, function(groupObj, dataIndex){
				var cellValue = groupObj.renderer ? groupObj.renderer(_self._getFieldValue(obj, dataIndex), obj) : _self._getFieldValue(obj, dataIndex);
				cellTempArray.push(['<span class="', CLS_GRID_GROUP_TITLE,'">', groupObj.title, groupObj.title ? ':' : '','</span><span class="', CLS_GRID_GROUP_TEXT,'">', cellValue, '</span>'].join(''));
			});

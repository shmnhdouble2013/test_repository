	...
	/** @lends  S.LP.GroupSelect.prototype */		
	{
		...
		*/
		...
		_setResult: function(data){
			...
			if(data){
				resultCon.val(S.JSON.stringify(data));
			}else{
				if(isTarget){
					_self._setResultFromTarget();
				}else{
		...
		_setResultFromSource: function(){
			...
			// 值为数组的序列化字符串
			if(resultCon){
				resultCon.val(S.JSON.stringify(_self.getSourceSelectedData()));
			}
		},
		// 根据目标列表的结果集 设置result
		_setResultFromTarget: function(){
			...
			// 值为数组的序列化字符串
			if(resultCon){
				resultCon.val(S.JSON.stringify(_self.getTargetData()));
			}
		}
	});
...
},{requires: ['1.0/grid', 'lpmodule/css/module.css']});

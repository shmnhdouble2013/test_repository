...
KISSY.add(function(S){
	...
	* ListTree 列表目录树
	* @memberOf S.LP
	* @description 列表目录树 基于treeBase 提供树的视图功能，并支持视图内搜索及整树搜索
	* @class ListTree 列表目录树
	* @param {Object} config 配置项 请参照TreeBase的配置项
	*/
	function ListTree(config){
		var _self = this;
		config = S.merge(ListTree.config, config);
		ListTree.superclass.constructor.call(_self, config);
		_self._init();
	}
	ListTree.config = 
	/** @lends  S.LP.ListTree.prototype */		
	{
		/**
		* 视图搜索框的提示文案 (选填)
		* @field
	...
	S.extend(ListTree, S.LP.TreeBase);
	S.augment(ListTree, 
	/** @lends  S.LP.ListTree.prototype */		
	{
		// 初始化

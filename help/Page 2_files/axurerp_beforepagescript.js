...
(function() {
    ...
    var _diagramObjectBase = {
        getStateStyleOverrides : function (state) {
            ...
            if (state == 'mouseDown') {
                var mouseOverStyle = styleObject.style &&
                    styleObject.style.stateStyles && styleObject.style.stateStyles.mouseOver;
                if (mouseOverStyle) {
                    // acount for the custom style property
                    var customStyle = mouseOverStyle.baseStyle &&
                        $axure.pageData.stylesheet.stylesById[mouseOverStyle.baseStyle];
                    if (customStyle) returnVal = $.extend(returnVal || { }, customStyle);
                    
                    returnVal = $.extend(returnVal || { }, mouseOverStyle);
                }
            }
            var stateStyle = styleObject.style &&
                styleObject.style.stateStyles && styleObject.style.stateStyles[state];
            if (stateStyle) {
                // acount for the custom style property
                var customStyle = stateStyle.baseStyle &&
                    $axure.pageData.stylesheet.stylesById[stateStyle.baseStyle];
                if (customStyle) returnVal = $.extend(returnVal || { }, customStyle);
                
                returnVal = $.extend(returnVal || { }, stateStyle);
            }
            return returnVal;
        }
    };
...
function InitializeSubmenu(submenudivid, tablecellid) {
    ...
    $(document.getElementById(tablecellid)).mouseenter(function (e) {
        ...
        if (e.pageX + 3 < offset.left || e.pageX > offset.left + subcontwidth || e.pageY + 3 < offset.top || e.pageY > offset.top + subcontheight) {
            ...
            SetWidgetOriginal(tablecellid, false);
        }
    });
...
$(document).ready(function(){
    $("div[tabindex='0']").click(function(){
        //alert("sdfsd");
        $("div[tabindex='0']").find("span").css("color","#666");
        $(this).find("span").css("color","#FF0000");
    })


    $("#u15_img").click(function(){
        location.href = "/";
    })
    $("#u28_img").click(function(){
        location.href = "/";
    })

 ...
 * update: 201401203 首页打开 二手房 || 租房的 类型选择
...
(function($, window){
    ...
    /* 二级菜单处理方法 start
    * 菜单 3种情况
    * 1、首页默认选中，hover其他一级菜单！ 相对应的二级菜单 有和无的 情况；----- 此时无二级 且 默认首页也没有二级 则隐藏二级菜单
    * 2、其他一级菜单 默认选中！ hover其他一级菜单，相对应的二级菜单 有和无的 情况；----- 此时无二级 则展现 默认选中的一级菜单相对应的 二级菜单, 若也没二级菜单 处理同1
    * 3、特殊情况，一级菜单没有默认选中！hover其他一级菜单，相对应的二级菜单 有和无的 情况 ----- 此时无二级 且 默认一级菜单 没有选中！则隐藏二级菜单； 同1
    * */
    ...
    // 菜单初始化
    initSecondMenu(INITSELECTFIRSTMENU);
...
//    }
    ...
    // 一级菜单 初始化
    function initSecondMenu(el){
        var elNode = el && $(el) || null,
            lia_Index = elNode && parseInt(elNode.attr(LIA_INDEX), 10),
            def_lia_Index = INITSELECTFIRSTMENU && parseInt(INITSELECTFIRSTMENU.attr(LIA_INDEX), 10);

        // 1、在没有任何一级菜单被选中情况下(二手房|租房 特殊情况 ) 隐藏所有二级菜单 容器
        // 2、默认 选中的一级菜单 有，但是没有二级菜单 并且当前 hove的 也没有 二级菜单！则直接隐藏二级菜单容器
        if(!elNode || !getNavUl(lia_Index) && !getNavUl(def_lia_Index) ){
            child_menu_content.hide();
        }else{
            showSecondMenuUl(elNode);
        }
    }
    ...
    // 地区选择 -- js控制 房屋类型选择 显示隐藏 || 暂时业务取消掉 不展开
    $('.city_box').mouseenter(function(){
        $('.hd_city_pop').show();
    }).mouseleave(function(){
        $('.hd_city_pop').hide();
    });
